...
$obj = json_decode($json);
		...
		if ($obj->unlikeqbtn==3)
		{
			...
			$query101=mysqli_query($conn,"SELECT * FROM `user_question_status` WHERE `q_id`='$q_id' and `user_id`='$user_id';");
			if (mysqli_num_rows($query101)==0)
			mysqli_query($conn,"INSERT INTO `user_question_status` (`q_id`,`user_id`,`status`) VALUES('$q_id','$user_id','UNLIKE');");
			else
			mysqli_query($conn,"UPDATE `user_question_status` set `status`='UNLIKE' WHERE `q_id`='$q_id' and `user_id`='$user_id';");
			
		}
		
	else if ($obj->unlikeqbtn==2)
		{
			$q_id=$obj->id;
			$user_id=$obj->sessionuserid;
			$query103=mysqli_query($conn,"SELECT * FROM `user_question_status` WHERE `q_id`='$q_id' and `user_id`='$user_id' and `status`='LIKE';");
			$query2=mysqli_query($conn,"SELECT * FROM `user_question` WHERE `q_id`='$q_id';");
			$fetch2=mysqli_fetch_assoc($query2);
			$userid1=$fetch2['user_id'];
			if (mysqli_num_rows($query103)==0)
			mysqli_query($conn,"DELETE FROM `user_question_status` where `user_id`='$user_id' and `q_id`='$q_id' and status='UNLIKE';");
			else
			{	
			...
			$message=$name1." has downvoted a question";
	...
	$query_status=mysqli_query($conn,"SELECT * FROM `user_question_status` where `q_id`='$q_id' and `user_id`='$user_id';");
		  ...
		  $no_of_unlike=mysqli_num_rows($query_unlike);
		   $list6[]=array('likeqbtn'=>$likebtn,'unlikeqbtn'=>$unlikebtn,'statuslike'=>$statuslike,'statusunlike'=>$statusunlike,'noof_likes'=>$no_of_like,'noof_unlikes'=>$no_of_unlike);
