...
if ($obj->likeabtn==3)
		{	
			...
			$query104=mysqli_query($conn,"SELECT * FROM `user_answer_status` WHERE `q_id`='$q_id' and `user_id`='$user_id' and `answer_id`='$answer_id' and `status`='UNLIKE';");
			...
			if (mysqli_num_rows($query104)==0)
			{
				mysqli_query($conn,"INSERT INTO `user_answer_status` (`user_id`, `answer_id`, `status`, `q_id`) VALUES('$user_id','$answer_id','LIKE','$q_id');");
...
else if ($obj->likeabtn==2)
		{
			$q_id=$obj->qid;
			$user_id=$obj->sessionuserid1;
			$answer_id=$obj->answerid;
			//echo '<script>alert("Posting in database")</script>';
			$query106=mysqli_query($conn,"SELECT * FROM `user_answer_status` WHERE `q_id`='$q_id' and `user_id`='$user_id' and `answer_id`='$answer_id' and `status`='UNLIKE';");
			if (mysqli_num_rows($query106)==0)
			mysqli_query($conn,"DELETE FROM `user_answer_status` where `user_id`='$user_id' and `q_id`='$q_id' and `answer_id`='$answer_id' and status='LIKE';");
			else
	...
	$list6[]=array('likeabtn'=>$answerlikebtn,'unlikeabtn'=>$answerunlikebtn,'answerstatuslike'=>$answerstatuslike,'answerstatusunlike'=>$answerstatusunlike,'noofanswer_like'=>$no_of_answer_like,'noofanswer_unlike'=>$no_of_answer_unlike);
	echo json_encode($list6);
?>