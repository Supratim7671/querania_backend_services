...
{
//echo '<script>alert("'.$verificationkey.'.'.$email.'");</script>';

$query=mysqli_query($conn,"SELECT * FROM `users` WHERE `resetkey`='$verificationkey' and `email`='$email';");
...
if ($numquery>0)
{
	...
	mysqli_query($conn,"UPDATE `users` set `verified`='YES' WHERE `resetkey`='$verificationkey' and `email`='$email';");
	
	echo '<script>window.onload=function(){$(\'#accountverified\').show();}</script>';	
	//echo'<script>setTimeout(function(){window.location.href=\'login_signup.html\'},5000);</script>';
	echo "<meta content=\"5;login_signup.html\" http-equiv=\"refresh\">";
}
else 
{	//echo '<script>alert("In the fifth condition");</script>';
	echo '<script>window.onload=function(){$(\'#verificationkeynotvalid\').show();}</script>';
}
}
...
<button class="w3-btn w3-btn-block w3-section w3-blue w3-ripple w3-round-large w3-card-2" type="submit" name="verify"> Verify Email </button>
</p>
</form>
	</div>
...
<div class="w3-modal" id="accountverified" role="dialog">
    <div class="w3-modal-content w3-card-8 w3-animate-zoom" style="max-width:500px;max-height:400px;">
      <div class="modal-content">
        ...
        <span onclick="document.getElementById('accountverified').style.display='none'" 
      class="w3-closebtn">&times;</span>
        <h2 class="w3-center">&nbsp;SUCCESS</h2>
      </header>
        <div class="w3-container">
          <p style="color:#A9A9A9;">Your account has been verified successfully.</p>
        </div>
				</div>
			</div>
			</div>
			
			<div class="w3-modal" id="verificationkeynotvalid" role="dialog">
    <div class="w3-modal-content w3-card-8 w3-animate-zoom" style="max-width:500px;max-height:400px;">
      <div class="modal-content">
        <header class="w3-center w3-container w3-teal"> 
        <span onclick="document.getElementById('verificationkeynotvalid').style.display='none'" 
      class="w3-closebtn">&times;</span>
        <h2 class="w3-center">&nbsp;FAILURE</h2>
      </header>
        <div class="w3-container">
          <p style="color:#A9A9A9;">Verification key is not valid.</p>
        </div>
				</div>
			</div>
			</div>
...
<footer class="w3-container w3-hide-small w3-hide-medium w3-theme-d5" style="position:fixed;bottom:0px;width:1400px;">
  <p>Powered by <a href=#" target="_blank">scholarsgo.com</a></p>
</footer>
<footer class="w3-container w3-hide-large w3-theme-d5" style="position:fixed;bottom:0px;width:400px;">
  <p>Powered by <a href=#" target="_blank">scholarsgo.com</a></p>
</footer>
